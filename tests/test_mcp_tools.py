...
from tools.search_docs import search_esp_docs, _load_resources
...
@patch("tools.search_docs._load_resources")
...
@patch("tools.search_docs._load_resources")
@patch("tools.search_docs.encoder")
@patch("tools.search_docs.faiss_index")
@patch("tools.search_docs.meta_data", new=[{"source_name": "Test TRM", "page": 5, "url": "http", "tags": [], "text": "ADC2 conflict"}])
...
def test_load_resources_missing(mock_exists):
    # Ensure it returns False if .cache is missing
    mock_exists.return_value = False
    assert _load_resources() == False

@patch("os.path.exists")
@patch("pickle.load")
@patch("builtins.open", new_callable=MagicMock)
def test_load_resources_mocked_success(mock_open, mock_pickle, mock_exists):
    # Skip actual faiss and sentence_transformers imports
    import sys
    ...
    import tools.search_docs
    # ensure we reset global states
    tools.search_docs.faiss_index = None
    tools.search_docs.meta_data = None
    tools.search_docs.encoder = None

    res = tools.search_docs._load_resources()
    assert res == True
    assert tools.search_docs.faiss_index is not None
